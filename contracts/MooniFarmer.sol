...
contract MooniFarmer is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        asteriskPerBlock =_asteriskPerBlock;
    ...
    function pendingMooni(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accumulatedMooni = pool.accumulatedMooni;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 mooniReward = multiplier.mul(mooniPerBlock).mul(pool.lpRewardWeight);
            accumulatedMooni = pool.accumulatedMooni.add(mooniReward);
        }
        return accumulatedMooni.mul(user.amount.div(1e9)).sub(user.rewardDebtForMooni);
    }
    ...
    function setAsteriskBurnRate(uint256 _burnRateDay, uint256 _burnRateNight) external onlyOwner() {
        ...
        asterisk.setBurnRate(_burnRateDay, _burnRateNight);
    }

    function setRewardCountPerBlock(uint256 _mooniPerBlock, uint256 _asteriskPerBlock) public {
        mooniPerBlock = _mooniPerBlock;
        asteriskPerBlock = _asteriskPerBlock;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            {
                safeMooniTransfer(msg.sender, pool.accumulatedMooni.mul(user.amount.div(1e9)).sub(user.rewardDebtForMooni));
                user.rewardDebtForMooni = user.amount.div(1e9).mul(pool.accumulatedMooni);
            }
            else{
                if(pending > 0) {
